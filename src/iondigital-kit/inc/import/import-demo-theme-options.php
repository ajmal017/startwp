...
if ( $Iondigital_importerError !== false ) {
	...
	if ( ! is_file( $import_filepath . '.php' ) ) {
		$response['id'] = new WP_Error( 'import_theme_options_nofile', 'The PHP file containing the theme options data could not be found or could not be read in <pre>' .  $import_filepath . '</pre> You might want to try to set the file permission to 777.<br/>If this doesn\'t work please use the Theme Options Import/Export and import the .txt file provided in the archive you\'ve received on purchase manually.' );
	} else {
		ob_start();
