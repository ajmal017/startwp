...
class Iondigital_import extends Iondigital_WP_Import {
	var $preStringOption;
	var $results;
	var $getOptions;
	var $saveOptions;
	...
	function import_posts_pages( $xml_file, $option_file, $stepNumber = 1, $numberOfSteps = 1 ) {
		...
		if ( $stepNumber == $numberOfSteps ) {
			...
			update_option( 'iondigital_demo_data_imported', true );
	...
	function process_posts_stepped( $stepNumber = 1, $numberOfSteps = 1 ) {
		...
		foreach ( $currentPosts as $post ) {
			...
			if ( ! post_type_exists( $post['post_type'] ) ) {
				printf( __( 'Failed to import "%s": Invalid post type %s', 'iondigital-kit' ), esc_html( $post['post_title'] ), esc_html( $post['post_type'] ) );
				echo '<br />';
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				//printf( __('%s &#8220;%s&#8221; already exists.', 'iondigital_kit'), $post_type_object->labels->singular_name, esc_html($post['post_title']) );
				//echo '<br />';
				...
				if ( is_wp_error( $post_id ) ) {
					printf( __( 'Failed to import %s "%s"', 'iondigital_kit' ), $post_type_object->labels->singular_name, esc_html( $post['post_title'] ) );
				...
				foreach ( $post['terms'] as $term ) {
					...
					if ( ! $term_id ) {
						...
						if ( ! is_wp_error( $t ) ) {
							...
							printf( __( 'Failed to import %s %s', 'iondigital_kit' ), esc_html( $taxonomy ), esc_html( $term['name'] ) );
							if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
	...
	function import_end() {
		...
		//echo '<p>' . __( 'All done.', 'iondigital_kit' ) . ' <a href="' . admin_url() . '">' . __( 'Have fun!', 'iondigital_kit' ) . '</a>' . '</p>';
		//echo '<p>' . __( 'Remember to update the passwords and roles of imported users.', 'iondigital_kit' ) . '</p>';
	...
	function process_menus() {
		...
		foreach ( $this->posts as $post ) {
			...
			//			if ( ! post_type_exists( $post['post_type'] ) ) {
			//				printf( __( 'Failed to import &#8220;%s&#8221;: Invalid post type %s', 'iondigital_kit' ),
	...
	function set_menus( $option_file ) {
		...
		$Iondigital_menus = wp_get_nav_menus();
		...
		if ( ! empty( $Iondigital_menus ) && ! empty( $menu_conf ) ) {
			foreach ( $Iondigital_menus as $Iondigital_menu ) {
				//check if we got a menu that corresponds to the Menu name array ($Iondigital_config->get('nav_menus')) we have set in menus.php
				if ( is_object( $Iondigital_menu ) && in_array( $Iondigital_menu->name, $menu_conf ) ) {
					$key = array_search( $Iondigital_menu->name, $menu_conf );

					if ( $key !== false ) {
						//if we have found a menu with the correct menu name apply the id to the menu location
						$locations[ $key ] = $Iondigital_menu->term_id;
					}
				}
			}
		}
			...
			foreach ( $import_widgets as $import_widget ) :
				...
				if ( array_key_exists( $title, $new_widgets ) ) {
					...
					if ( ! empty( $new_widgets[ $title ]['_multiwidget'] ) ) {
						...
						$new_widgets[ $title ]['_multiwidget'] = 1;
					}
					if($title == 'text'){
			
						$output[] = $new_widgets[ $title ];
