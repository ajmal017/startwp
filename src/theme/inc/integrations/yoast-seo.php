 ...
 * WE ensure that Bitstarter plays nicely with YOAST SEO
 * See: https://wordpress.org/plugins/wordpress-seo/
 *
 * @package Bitstarter
 */

if ( ! function_exists( 'bitstarter_yoast_listing_image' ) ) {
	function bitstarter_yoast_listing_image( $metadata, $object_id, $meta_key, $single ) {
		...
		if ( null === $metadata && empty( $meta_key ) && false === $single && 'job_listing' == get_post_type( $object_id ) && ! has_post_thumbnail( $object_id ) ) {
			remove_filter( 'get_post_metadata', 'bitstarter_yoast_listing_image', 100 );
			$current_meta = get_post_meta( $object_id, '', false );
			add_filter( 'get_post_metadata', 'bitstarter_yoast_listing_image', 100, 4 );

			if ( empty( $current_meta[ WPSEO_Meta::$meta_prefix . 'twitter-image' ][0] ) ) {
				//get the first image in the listing's gallery
				$photos = bitstarter_get_listing_gallery_ids();
				if ( ! empty( $photos[0] ) ) {
